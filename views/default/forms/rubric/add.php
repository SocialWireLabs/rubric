...
if (!elgg_is_sticky_form('add_rubric')) {
   $title = "";
   $tags = "";
   $access_id = 0;
   $rows = 4;
   ...
   $title = elgg_get_sticky_value('add_rubric','title');
   $tags = elgg_get_sticky_value('add_rubric','rubrictags');
   $access_id = elgg_get_sticky_value('add_rubric','access_id');
   $rows = elgg_get_sticky_value('add_rubric','rows');
   $cols = elgg_get_sticky_value('add_rubric','cols');
   $criteria_value = elgg_get_sticky_value('add_rubric','criteria_value');
   $criteria_name = elgg_get_sticky_value('add_rubric','criteria_name');
   $criteria_desc = elgg_get_sticky_value('add_rubric','criteria_desc');
   $level_value = elgg_get_sticky_value('add_rubric','level_value');
   $level_desc = elgg_get_sticky_value('add_rubric','level_desc');
   $criteria_level_desc = elgg_get_sticky_value('add_rubric','criteria_level_desc');
   $first = false;
}
...
?>
<form action="<?php echo $url."action/".$action?>" name="add_rubric" enctype="multipart/form-data" method="post">
...
<?php
$enter_criteria_value = elgg_echo('rubric:enter_criteria_value');
...
function addcol() {
   ...
   for (i=0; i<numfilas; i++) {
      ...
      if (i==0) {
         n = numcols+1;
         nodo.innerHTML = '<td id="celda'+i+n+'" class="celdaarriba"><table id="tcelda'+i+n+'" border="1"><tr><td><input type="text" id="texto'+i+n+'" name = "level_desc[]" value="$enter_level_description" size="22"></td></tr><tr><td id="puntos"><center><input id= "nombre'+i+n+'" type="text" size="4" value="$enter_level_value" name="level_value[]"></center></td></tr></table></td>';
      } else {
